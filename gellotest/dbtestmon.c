...
int main(int argc, char *argv[])
{
	...
	if(argc < 2)
		g_err(EXIT,NOPERROR,"Usage: %s portnum",argv[0]);
...
while(true){
	...
	numpacks=0;
        ...
        {
		numpacks++;
            /* Receive up to the buffer size a null terminator) bytes from the sender */
	    totalbytes=0;
	    bytesRcvd=0;
	    while(totalbytes != DBPACKETSIZE){
		    if ((bytesRcvd = recv(clntSock, echoBuffer+totalbytes, DBPACKETSIZE-totalbytes,MSG_NOSIGNAL)) < 0)
			g_err(EXIT,PERROR,"recv() critical failure");
		    else if(bytesRcvd == 0){
		    ...
		    if(totalbytes < DBPACKETSIZE){
				...
				"Partial packet# %d of %d/%d totalbytes on receive",numpacks,bytesRcvd,totalbytes); 
			    g_err(NOEXIT,NOPERROR,"[%.*s]\n",totalbytes,echoBuffer);
		    }
	    }
            echoBuffer[totalbytes] = '\0';  /* Terminate the string! */
	    if(echoBuffer[199] != '}'){
		    g_err(NOEXIT,NOPERROR,"Bad fing packet");
		    g_err(NOEXIT,NOPERROR,"strlen: %d\n",strlen(echoBuffer));
	    }
	    g_err(NOEXIT,NOPERROR,"\npacknum:%d:totalbytes:%d:[ %s ]\n", numpacks,totalbytes,echoBuffer);
...
docommand(int clntsock, char *buf){
	...
	switch(*buf){
		...
		case 'A':
			...
			len=sprintf(locbuf,"SETIP 18.222.197.43 32159");
			locbuf[len]=PADCHAR;
			break;
		case 'B':
			//GETIP

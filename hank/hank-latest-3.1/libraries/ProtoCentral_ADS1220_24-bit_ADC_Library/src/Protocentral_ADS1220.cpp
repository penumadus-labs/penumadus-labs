...
void Protocentral_ADS1220::begin(uint8_t cs_pin, uint8_t drdy_pin)
{
	...
	//use 8Mhz clock 
	//prototype SPI.beginTransaction(SPISettings(8000000 ,MSBFIRST, SPI_MODE1));
...
int32_t Protocentral_ADS1220::Read_WaitForData()
{
    ...
    //ADDED by gello
    //checks for 10mS (100*100 uS)
    //for sample to be ready then aborts
    //at 3sample/second this should be plenty
    int i;
    for(i=0;i<100;i++){
    	if((digitalRead(m_drdy_pin)) == LOW)
		break;
	else

...
struct networktime_t {
	...
	networktime = {0,0,0};	//network time from server
...
loop() {
	...
	if(timerExpired(TRIGGERSERVER)){
			...
			/* first character D to represent regular data in binary */
			updateRealTimeClock();
			len=sprintf(spbuf,"D %s %s %d.%03d %d %d %d %ld %ld\n",
				tankInit.deviceId,
				tankInit.deviceReg,
				(int)pres,
				abs(((int)(pres*1000))%1000),
				tankInit.fill,
				temp,
				hum,
				networktime.secs,
...
process_accelerometer(void){
	...
	if( mag > tankInit.maxg ) {
		updateRealTimeClock();
		/* send the data,  first character A to represent Acceleration data in binary */
		len=sprintf(spbuf,
			"A %s %s %d.%02d %d.%02d %d.%02d %d.%02d %ld %ld\n",
			tankInit.deviceId,
			tankInit.deviceReg,
			(int)mag,
			((int)(mag*100))%100,
...
/* update the seconds when microseconds roll over */
...
resynctime(const char *timeptr){
	...
	sprintf(spbuf,"Time Synced: %ld %ld",networktime.secs,networktime.usecs);
	Serial.println(spbuf);
}

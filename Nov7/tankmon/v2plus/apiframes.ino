...
checkReceivedFrames(unsigned char *frameptr){
	...
	switch(frameptr[FRAMETYPE]){
		case ATCMNDRESP:
			Serial.println(F("AT cmnd response"));
		...
		case MODEMSTATUS:
			...
			switch(frameptr[4]){
				case 0:
					Serial.println(F("Hardware reset or power up"));break;
				case 1:
					Serial.println(F("Watchdog timer reset"));break;
				case 2:
					Serial.println(F("Registered with cellular network"));break;
				case 3:
		...
		case APITXREQRESP:
			Serial.println(F("TX cmnd response"));
			/* is upper nibble equal to cellstat ids */
			if((frameptr[FRAMEID])==packetpending){
				packetpending=0;
				switch(frameptr[FRAMEID+1]){
					case 0x0: 
						Serial.println(F("Succesful Transmit"));
						break;
					case 0x21: 

...
//enqueueL('D',char *msg)
	...
	/* need to start queueing to sd card? */
		...
		//PONDSCUM TEST
		Serial.println(F("Start SD queueing"));
		qtoSD=true;
		enqBufptr=lbuf;
	}
	...
	/* queue it up */
		...
		enqBufptr=elements[mybuf].buffer;
	}

	updateRealTimeClock();

	/* put formatted data in char buffer */
	switch(type){
		case 'D':
			len=prepdataD(enqBufptr,
				(float *)arg1,
				(int *)arg2,
				(int *)arg3,
				(int *)arg4);
		...
		case 'L':
		case 'C':
			len=snprintf(enqBufptr,MEMELEMENTDATASIZE,"%c %s",type,(char *)arg1);
			break;
		default:
			len=0;
			break;
	...
	if(len >=MEMELEMENTDATASIZE){
		Serial.println(enqBufptr);
		sprintf(enqBufptr,"L q str 2 long");
		Serial.println(enqBufptr);
	}

	if(qtoSD){
		//PONDSCUM TEST
		sprintf(spbuf," S%x",globbufcounter++);
		strcpy(enqBufptr+strlen(enqBufptr),spbuf);
		...
		//PONDSCUM TEST
		sprintf(spbuf," Q%x",globbufcounter++);
		strcpy(enqBufptr+strlen(enqBufptr),spbuf);

		appendToList(list,mybuf);
		//Serial.print(F("new buf: "));
		//Serial.println(enqBufptr);
		Serial.println(elements[mybuf].buffer);
		return(true);
	}
}

int
prepdataD(char *buf, float *pres, int *fills, int *temp, int *hum){
	int len;
	len=snprintf(buf,MEMELEMENTDATASIZE,
		"D %d.%02d %d %d %d %lx %lx",
		(int)(*pres),
...
int
prepdataA(char *buf, float *arrx, float *arry, float *arrz){
	int len;
	len=snprintf(buf,MEMELEMENTDATASIZE,
		"A %d.%02d %d.%02d %d.%02d %lx %lx",
		(int)(*arrx),
		abs(((int)((*arrx)*100))%100),
		(int)(*arry),
		abs(((int)((*arry)*100))%100),
		(int)(*arrz),
		abs(((int)((*arrz)*100))%100),
		networktime.secs,
		networktime.usecs
	);
	return(len);
...
sendNextQPacket(){
	...
	/* if not wifi, send cell if available */
		...
		if(oldestinQ == 0xFF)
			return(false);
		bufptr= elements[oldestinQ].buffer;

		len=makefullpacket(bufptr,(char *)lbuf);
		...
		if(qtoSD && (freelistcount > FREELISTHIGHWATER)){
			//PONDSCUM TEST
			Serial.println(F("SD queuing off"));
			...
			if(fd_SD=SD.open(SDBUFFILE)){
				//PONDSCUM TEST
				Serial.println(F("read open SD file"));
		 ...
		 */
		if(qtoSD && (sendqueue.numelements != 0) ){
...
process_accelerometer(void){
	...
	if(!tracking){
		...
		if( (mag > tankInit.maxg) ) {
			//PONDSCUM TEST
			Serial.println("APP old samples to list");
			...
			if(accelqueue.numelements >= BACKSAMPLES){
			...
			else if((tmp=allocMemElement())==0xFF) {
				...
				if( (tmp=removeFirstElement(&accelqueue)) == 0xFF){
					/* just plain hosed, mem is locked, drop any predata on floor */ 
					return;
				}
			}

			/* at this point, we have a buffer from somewhere */
			len=prepdataA(elements[tmp].buffer, &arrx, &arry, &arrz);
			/* exceeded buffer size,  queue up an error message to log */
			if(len >=MEMELEMENTDATASIZE){
				char *bufptr;
				bufptr=elements[tmp].buffer;
				Serial.println(bufptr);
				sprintf(bufptr,"L q str 2 long");
				Serial.println(bufptr);
				appendToList(&sendqueue,tmp);
			}
			else{
...
queuetoSD(char *bufptr){
	...
	len=makefullpacket(bufptr,lbuf);
...
int
makefullpacket(char *bufptr,char *outbufptr){
	int len;
	...
	switch(*bufptr){
		...
		/* accelerometer data */
		case 'A':
			len=snprintf(outbufptr,FRAMESIZE,"%c %s %s %s\n",
				*bufptr,
				tankInit.deviceId,
				tankInit.deviceReg,
				bufptr+2
		...
		/* command packet to server */
		/* PONDSCUM perhaps a periodic time sync command somewhere */
		case 'C':
		case 'L':
			len=snprintf(outbufptr,FRAMESIZE,"%c %s\n",
				*bufptr,
				bufptr+2
			);
			break;
		...
		default:
			...
			Serial.println(*bufptr);
			len=snprintf(outbufptr,FRAMESIZE,"L Bad Pck in Q %s\n",bufptr);
			break;
	}
	return(len);
}
...
checkSDqueue(void){
	...
	if(fd_open && !fd_SDwriteMode){
		if(fd_SD.available()){
			...
			fd_open=false;
			dataONSD=false;
			Serial.println(F("end SD Q, remove"));
			SD.remove(SDBUFFILE);
		}
